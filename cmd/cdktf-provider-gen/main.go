...
var gen = &cli.App{
	...
	Action: func(c *cli.Context) error {
		...
		if config.Target == nil {
			return errors.New("language target config is required")
		}
		if config.Target.Go == nil {
			return errors.New("go target config is required")
		}

		if config.Target.Go.PackageName == "" {
			config.Target.Go.PackageName = config.Name
		}
		...
		logger.Debug("copying output", log.String("outputDir", outputDir))
		...
		if err := cp.Copy(filepath.Join(tmpDir, "dist", "go", config.Target.Go.PackageName), outputDir); err != nil {
